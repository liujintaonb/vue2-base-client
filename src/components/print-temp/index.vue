  ...
  <div class="print-wrap is-print">
    ...
    <main class="print-wrap__main">
      <section class="print-text print-text--right">
        ...
        <span class="active-text width-60">{{ detail.daYinXh }}</span>
        号
      </section>
      ...
      <section class="print-text">
        ...
        <span class="active-text width-464">
          {{ detail.gpsDbx }},{{ detail.gpsDby }}; {{ detail.gpsDnx }},{{
            detail.gpsDny
          }}; {{ detail.gpsXnx }},{{ detail.gpsXny }}; {{ detail.gpsXbx }},{{
            detail.gpsXby
          }};
        </span>
      </section>
      ...
      <section class="print-text">
        ...
        <span class="active-text width-50">{{
          handleDate(detail.caiFaQxq)[0]
        }}</span>
        年
        <span class="active-text width-40">{{
          handleDate(detail.caiFaQxq)[1]
        }}</span>
        月
        <span class="active-text width-40">{{
          handleDate(detail.caiFaQxq)[2]
        }}</span>
        日至
        <span class="active-text width-50">{{
          handleDate(detail.caiFaQxz)[0]
        }}</span>
        年
        <span class="active-text width-40">{{
          handleDate(detail.caiFaQxz)[1]
        }}</span>
        月
        <span class="active-text width-40">{{
          handleDate(detail.caiFaQxz)[2]
        }}</span>
        日
      </section>
      <section class="print-text">
        ...
        <span class="active-text width-50">{{
          handleDate(detail.gengXingQx)[0]
        }}</span>
        年
        <span class="active-text width-40">{{
          handleDate(detail.gengXingQx)[1]
        }}</span>
        月
        <span class="active-text width-40">{{
          handleDate(detail.gengXingQx)[2]
        }}</span>
        日
      </section>
      <section class="print-text">
      ...
      <section class="print-text">
        ...
        <span
          class="active-text active-desc"
          style="padding-left: 40px"
          v-html="detail.erWeiMaBeiZhu"
        ></span>
      </section>
    </main>
    ...
    <div class="print-text lh-20" style="text-indent: 28px">
      2.非国有林木采伐不填写GPS定位。
    </div>
  </div>
</template>
