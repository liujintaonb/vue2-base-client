...
import { getXzCodeBySg } from "@/common/baseApi/api";
export default {
  name: "Xdanwei",
  props: {
    value: {
  ...
  methods: {
    ...
    async initByXzCode(xzCode = "") {
      ...
      if (!xzCode) {
        if (this.commissId) {
          xzCode = this.provinceOptions[0].c_frcode;
